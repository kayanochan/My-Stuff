...
int main(int argc, char *argv[]) {
    ...
    ireplay = al_load_bitmap("replay.bmp");
    if (!ireplay) {
            al_show_native_message_box(display, "Error", "Error",
                                       "Failed to load graphics!",
                                       nullptr, ALLEGRO_MESSAGEBOX_ERROR);
	...
	// Buttons
	Button title, play, hi, bback, replay, levels[5], card3[9], card4[16], card5[25], card6[36], card7[49];
	make(title, disp_data.width/4, disp_data.height/100, 3*disp_data.width/4, 109*disp_data.height/900, 1, ititle);
	make(play, 3*disp_data.width/8, 4*disp_data.height/9, 5*disp_data.width/8, 2*disp_data.height/3, 1, iplay);
	make(hi, disp_data.width/3, 791*disp_data.height/900, 2*disp_data.width/3, 99*disp_data.height/100, 1, ihi);
	make(bback, disp_data.width/100, 791*disp_data.height/900, 3*disp_data.width/25, 99*disp_data.height/100, 0, iback);
	make(replay, disp_data.width/100, 10*disp_data.height/100, 3*disp_data.width/25, 199*disp_data.height/900, 0, ireplay);
	for (int i=0;i<5;i++) {
        make(levels[i], (1+(float)i*3/2)*disp_data.width/9, 4*disp_data.height/9,
             (2+(float)i*3/2)*disp_data.width/9, 2*disp_data.height/3, 0, ilevels[i]);
	}
    for (int i=0;i<9;i++) {
        make(card3[i], disp_data.width/2+disp_data.height*((float)(i%3)*166/459-(float)8/17),
             (float)(floor(i/3)*11/10+0.1)*disp_data.height*8/27,
             disp_data.width/2+disp_data.height*((float)(i%3)*166/459-(float)116/459),
             (float)(floor(i/3)*11/10+1.1)*disp_data.height*8/27, 0, icards[i%5]);
        card3[i].poker = ipoker;
	}
	for (int i=0;i<16;i++) {
        make(card4[i], disp_data.width/2+disp_data.height*((float)(i%4)*961/3645-(float)43/90),
             (float)(floor(i/4)*11/10+0.1)*disp_data.height*2/9,
             disp_data.width/2+disp_data.height*((float)(i%4)*961/3645-(float)761/2430),
             (float)(floor(i/4)*11/10+1.1)*disp_data.height*2/9, 0, icards[i%8]);
        card4[i].poker = ipoker;
	}
	for (int i=0;i<25;i++) {
        make(card5[i], disp_data.width/2+disp_data.height*((float)(i%5)*629/3024-(float)27/56),
             (float)(floor(i/5)*11/10+0.1)*disp_data.height*8/45,
             disp_data.width/2+disp_data.height*((float)(i%5)*629/3024-(float)529/1512),
             (float)(floor(i/5)*11/10+1.1)*disp_data.height*8/45, 0, icards[i%10]);
        card5[i].poker = ipoker;
	}
	for (int i=0;i<36;i++) {
        make(card6[i], disp_data.width/2+disp_data.height*((float)(i%6)*311/1809-(float)65/134),
             (float)(floor(i/6)*11/10+0.1)*disp_data.height*4/27,
             disp_data.width/2+disp_data.height*((float)(i%6)*311/1809-(float)1355/3618),
             (float)(floor(i/6)*11/10+1.1)*disp_data.height*4/27, 0, icards[i%12]);
        card6[i].poker = ipoker;
	}
	for (int i=0;i<49;i++) {
        make(card7[i], disp_data.width/2+disp_data.height*((float)(i%7)*463/3159-(float)19/39),
             (float)(floor(i/7)*11/10+0.1)*disp_data.height*8/63,
             disp_data.width/2+disp_data.height*((float)(i%7)*463/3159-(float)413/1053),
             (float)(floor(i/7)*11/10+1.1)*disp_data.height*8/63, 0, icards[i%25]);
	...
	while (!doexit) {
        ...
        // Update the screen
            ...
            if(t) {
                al_draw_textf(font, BLACK, disp_data.width/100, disp_data.height/100, 0, "%.2f", (float)t/FPS);
            }
            if(game>10){
                for(int i=0;i<10;i++){
            ...
            if (game>0&&game<10&&!check&&stop<=0) {
                t++;
            }
            else if (!check){
                t=0;
      	...
      	if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) {
            ...
            if (click(replay, mouse.x, mouse.y, stop)) {
                t=0;
                check=0;
                for(int i=0;i<9;i++){
                    card3[i].side=0;
            ...
            if ((click(levels[0], mouse.x, mouse.y, stop)&&levels[0].side==1)||
                (click(replay, mouse.x, mouse.y, stop)&&game==3)) {
                title.side = 0;
                replay.side=0;
                for(int i=0;i<5;i++){
                    levels[i].side=0;
            ...
            if ((click(levels[1], mouse.x, mouse.y, stop)&&levels[1].side==1)||
                (click(replay, mouse.x, mouse.y, stop)&&game==4)) {
                title.side = 0;
                replay.side=0;
                for(int i=0;i<5;i++){
                    levels[i].side=0;
            ...
            if ((click(levels[2], mouse.x, mouse.y, stop)&&levels[2].side==1)||
                (click(replay, mouse.x, mouse.y, stop)&&game==5)) {
                title.side = 0;
                replay.side=0;
                for(int i=0;i<5;i++){
                    levels[i].side=0;
            ...
            if ((click(levels[3], mouse.x, mouse.y, stop)&&levels[3].side==1)||
                (click(replay, mouse.x, mouse.y, stop)&&game==6)) {
                title.side = 0;
                replay.side=0;
                for(int i=0;i<5;i++){
                    levels[i].side=0;
            ...
            if ((click(levels[4], mouse.x, mouse.y, stop)&&levels[4].side==1)||
                (click(replay, mouse.x, mouse.y, stop)&&game==7)) {
                title.side = 0;
                replay.side=0;
                for(int i=0;i<5;i++){
                    levels[i].side=0;
...
int click(Button button, float clickx, float clicky, int stop) {
    if(clickx >= button.x1 && clicky >= button.y1 &&
        clickx <= button.x2 && clicky <= button.y2 &&
        button.side && stop<=0){
        return 1;
    }
    else{
...
void draw(Button button) {
    ...
    if(button.side==3){
        al_draw_scaled_bitmap(button.poker, 0, 0,
                              ...
                              al_get_bitmap_height(button.poker),
                              button.x1, button.y1,
                              button.x2-button.x1, button.y2-button.y1, 0);
    }
    else if(button.side){
        al_draw_scaled_bitmap(button.img, 0, 0,
                              al_get_bitmap_width(button.img),
                              al_get_bitmap_height(button.img),
                              button.x1, button.y1,
                              button.x2-button.x1, button.y2-button.y1, 0);
    }
